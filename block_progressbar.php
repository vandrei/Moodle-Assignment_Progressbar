    ...
    public function get_content() {
        ...
        if ($nr_enrolled_courses > 0) {
            ...
            foreach ($courses_context as $course) {
                ...
                $allow_subs = $DB->get_fieldset_select('assign', 'allowsubmissionsfromdate',
                        "course = $course_no AND duedate > $crr_time", null);
                ...
                if (is_array($due_dates)) {
                    for ($i=0; $i < count($due_dates); $i++) {
                        ...
                        if ($assgn_prog <= 50) {
                            $this->content->text .= "$course_name - $hw_name[$i] - $assgn_prog%
                                    <div class=\"progress progress-striped active\">
                                    <div class=\"bar bar-success\" style=\"width: $assgn_prog%;\"></div></div>";
                        } else {
                            if ($assgn_prog <= 80) {
                                $this->content->text .= "$course_name - $hw_name[$i] - $assgn_prog%
                                        <div class=\"progress progress-striped active\">
                                        <div class=\"bar bar-warning\" style=\"width: $assgn_prog%;\"></div></div>";
                            } else {
                                $this->content->text .= "$course_name - $hw_name[$i] - $assgn_prog%
                                        <div class=\"progress progress-striped active\">
                                        <div class=\"bar bar-danger\" style=\"width: $assgn_prog%;\"></div></div>";
                            }
                        }
                    }
                } else {
                    ...
                    if ($due_dates) {
                        ...
                        if ($assgn_prog <= 50) {
                            $this->content->text .= "$course_name - $hw_name[$i] - $assgn_prog%
                                    <div class=\"progress progress-striped active\">
                                    <div class=\"bar bar-success\" style=\"width: $assgn_prog%;\"></div>$hw_name</div>";
                        } else {
                            if ($assgn_prog <= 80) {
                                $this->content->text .= "$course_name - $hw_name[$i] - $assgn_prog%
                                        <div class=\"progress progress-striped active\">
                                        <div class=\"bar bar-warning\" style=\"width: $assgn_prog%;\"></div>$hw_name</div>";
                            } else {
                                $this->content->text .= "$course_name - $hw_name[$i] - $assgn_prog%
                                        <div class=\"progress progress-striped active\">
                                        <div class=\"bar bar-danger\" style=\"width: $assgn_prog%;\"></div>$hw_name</div>";
                            }
                        }
                    }
                }
